 ...
 * activates at a time
 */
export function createHyperSubLayers(subLayers: SubLayersType): KarabinerRules[] {
  const keys = Object.keys(subLayers) as (keyof typeof subLayers)[];
  const allSubLayerVariables = keys.map((sublayer_key) => generateSubLayerVariableName(sublayer_key));
  const subLayerVariableConditions = allSubLayerVariables.map((subLayerVariable) => ({
    type: "variable_if" as const,
    name: subLayerVariable,
    value: 0,
  }));
